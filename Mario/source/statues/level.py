...
class Level:
    ...
    def setup_background(self, screen):
        ...
        self.game_window = screen.get_rect()  # 获得显示窗口大小
        self.game_ground = pygame.Surface(
            (self.background_rect.width, self.background_rect.height))  # 新建一个背景大小般的图层
    ...
    def setup_ground_items(self):
        # 创建一个精灵组,把实例化的物品都放到这个组里，方便之后的批量处理
        self.ground_items_group = pygame.sprite.Group()
        for name in ['ground', 'pipe', 'step']:  # 大地，管道，楼梯
            for item in self.map_data[name]:
                self.ground_items_group.add(stuff.Item(
                    item['x'], item['y'], item['width'], item['height'], name))
    ...
    def setup_player(self):
        ...
        self.player.rect.x = self.game_window.x + \
            self.player_x  # 让角色所处矩形的位置为滑动窗口的位置加上角色的x坐标
    ...
    def setup_brick_and_box(self):
            ...
            for brick_data in self.map_data['brick']:
                ...
                if brick_type == 0:
                    if 'brick_num' in brick_data:
                        ...
                        self.brick_group.add(brick.Brick(
                            x, y, brick_type, None))  # 砖块精灵加入砖块组
                        # print('test',brick.Brick(x,y,brick_type))
                elif brick_type == 1:
                    self.brick_group.add(brick.Brick(
                        x, y, brick_type, self.coin_group))
                else:
                    self.brick_group.add(brick.Brick(
                        x, y, brick_type, self.powerup_group))
        if 'box' in self.map_data:
            for box_data in self.map_data['box']:
                ...
                if box_data == 1:
                    self.box_group.add(
                        box.Box(x, y, box_data, self.coin_group))
                else:
                    self.box_group.add(
                        box.Box(x, y, box_data, self.powerup_group))
                # elif box_data == 5:
    ...
    def setup_checkpoints(self):
        ...
        for item in self.map_data['checkpoint']:
            ...
            self.checkpoint_group.add(stuff.Checkpoint(
                x, y, w, h, checkpoint_type, enemy_groupid))
    ...
    def check_x_collisions(self):
        ...
        check_group = pygame.sprite.Group(
            self.ground_items_group, self.brick_group, self.box_group)  # 两个精灵组的精灵都加进新的精灵组
        ground_item = pygame.sprite.spritecollideany(self.player, check_group)

        player_colided_flag = pygame.sprite.spritecollideany(
            self.player, self.pole_group)
    ...
    def check_y_collisions(self):
        ...
        ground_item = pygame.sprite.spritecollideany(
            self.player, self.ground_items_group)
        ...
        if ground_item:
            ...
            if self.player.hurt_imune:
                return
            self.text_info_group.add(b.Text_info(
                enemy.rect.x, enemy.rect.y, '+ 100'))
        ...
        # upwards
            ...
            if sprite.name == 'brick':
                if sprite.brick_type == 1 and sprite.state == 'rest':
                    self.text_info_group.add(b.Text_info(
                        sprite.rect.x, sprite.rect.y, '+ 100'))
                    self.game_info['score'] += 100
                    self.game_info['coin'] += 1
    ...
    def check_will_fail(self, sprite):  # 坠落检测
        ...
        sprite.rect.y += 1
        check_ground = pygame.sprite.Group(
            self.ground_items_group, self.brick_group, self.box_group)
    ...
    def draw(self, surface):
        ...
        self.game_ground.blit(
            self.background, (self.game_window.x, self.game_window.y), self.game_window)
