...
class Player(pygame.sprite.Sprite):
    ...
    def load_data(self):
        ...
        file_path = os.path.join(
            'source', 'data', 'player', file_name)  # 使用该项目绝对定位
    ...
    def load_images(self):  # 载入主角的各种帧造型
        ...
        self.small_normal_frames = [
            self.rigth_small_normal_frames, self.left_small_normal_frames]  # 小形态各种帧造型
        self.big_normal_frames = [
            self.rigth_big_normal_frames, self.left_big_normal_frames]  # 大号正常
        self.big_fire_frames = [
            self.rigth_big_fire_frames, self.left_big_fire_frames]  # 大号喷火
        ...
        for group, group_frames_rects in frame_rects.items():  # 可遍历建值和建值对
            for frame_rect in group_frames_rects:
                right_image = tools.get_image(
                    sheet, frame_rect['x'], frame_rect['y'], frame_rect['width'], frame_rect['height'], (0, 0, 0), constants.PLAYER_MULTI)
                left_image = pygame.transform.flip(
                    right_image, True, False)  # 翻转
    ...
    def walk(self, keys, level):
       ...
       # print('时间差：',shijiancha)   #一直按着按键的时间差，33-34毫秒
        # 当时间差大于100是，切换Mario帧，速度越快摆臂也要越快.calc_frame_duration帧持久
        ...
        if keys[pygame.K_RIGHT]:
            ...
            # print(self.x_vel)
            self.x_vel = self.calc_vel(
                self.x_vel, self.x_accel, self.max_x_vel, True)
            ...
            if self.x_vel > 0:
                ...
                self.x_accel = self.turn_accel
            self.x_vel = self.calc_vel(
                self.x_vel, self.x_accel, self.max_x_vel, False)
        else:
            if self.face_rigth:
    ...
    def fall(self, keys, level):
        self.y_vel = self.calc_vel(
            self.y_vel, self.gravity, self.max_y_velocity)
        # 临时代码，便于记忆
    ...
    def small2big(self, keys):
        ...
        frames_and_idx = [(self.small_normal_frames, 0),
                          (self.small_normal_frames, 7), (self.big_normal_frames, 0)]
    ...
    def big2small(self, keys):
        ...
        frames_and_idx = [(self.small_normal_frames, 8),
                          (self.big_normal_frames, 8), (self.big_normal_frames, 4)]  # 帧库及其对应帧索引
        if self.transtion_timer == 0:
            self.big = False
            self.fire = False
            self.transtion_timer = self.current_time
    ...
    def big2fire(self, keys):
        ...
        frames_and_idx = [(self.big_fire_frames, 3),
                          (self.big_normal_frames, 3)]  # 帧库及其对应帧索引
        if self.transtion_timer == 0:
            self.fire = True
            self.transtion_timer = self.current_time
            self.changing_index = 0
    ...
    def livesAdd1(self, level):
        if self.levies_add_time == 0:
            ...
            y = self.rect.top-50
            level.info.lives_labels.append(
                (level.info.create_lable('lives + 1'), (252, y)))
            print('我来了3')

    def change_player_image(self, frames, index):
        self.frame_index = index
        if self.face_rigth:
            # 获取self.rigth_small_normal_frames,rigth_big_normal_frames
            self.right_frames = frames[0]
            # 按照传入的参数取出小mario，中Mario，大mario
            self.image = self.right_frames[self.frame_index]
        else:
    ...
    def calc_frame_duration(self):
        duration = -60/self.max_run_speed + \
            abs(self.x_vel)+80  # abs() 函数返回数字的绝对值
        return duration
    ...
    def shot_fireball(self, level):
        ...
        if self.current_time-self.last_fireball_timer > 300:
            ...
            fireball = powerup.Fireball(
                self.rect.centerx, self.rect.centery, self.face_rigth, level.game_info)
            # 加入精灵组，在level里直接调用update和draw即可显示
