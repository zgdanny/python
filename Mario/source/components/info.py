...
class Info():
    ...
    def create_coin_counter(self, coin):
        ...
        if coin == 0:
            ...
            self.coin_labels.append(
                (self.create_lable('X0{0}'.format(coin)), (300, 55)))
        else:
            self.coin_labels.clear()
            self.coin_labels.append(
                (self.create_lable('X{0}'.format(coin)), (300, 55)))
        ...
        if self.game_info['statue'] == 'level':
            ...
            if self.time == 402:
                ...
                self.time_labels.append(
                    (self.create_lable('{0}'.format(self.time)), (635, 55)))
                self.time -= 1
    ...
    def create_score_group(self, score):
        ...
        if score == 0:
            ...
            self.update_labels.append(
                (self.create_lable('000{0}'.format(score)), (75, 55)))
        elif score < 10000:
            self.update_labels.clear()
            self.update_labels.append(
                (self.create_lable('00{0}'.format(score)), (75, 55)))
        elif score < 100000:
            self.update_labels.clear()
            self.update_labels.append(
                (self.create_lable('0{0}'.format(score)), (75, 55)))
        else:
            self.update_labels.clear()
            self.update_labels.append(
                (self.create_lable('{0}'.format(score)), (75, 55)))

    def create_state_lable(self):  # 创造某阶段特有的文字
        self.state_labels = []
        if self.state == 'main_menu':
            self.state_labels.append(
                (self.create_lable('1 PLAYER GAME'), (272, 360)))
            self.state_labels.append(
                (self.create_lable('2 PLAYER GAME'), (272, 405)))
            self.state_labels.append(
                (self.create_lable('TOP   -'), (290, 465)))
            ...
            self.state_labels.append(
                (self.create_lable('X   {0}'.format(self.game_info['lives'])), (380, 280)))
            self.play_image = tools.get_image(
                setup.GRAPHICS['mario_bros'], 178, 32, 12, 16, (0, 0, 0), constants.BG_MULTI)
        elif self.state == 'game_over':
            self.state_labels.append(
                (self.create_lable('GAME OVER'), (280, 300)))
